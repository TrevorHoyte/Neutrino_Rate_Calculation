...
def Sigma_max(E_n): #insert energy of incident neutrio
...
#function computes cross section using 2009 model
def Sigma_min(E_n): #insert energy of incident neutrio
    #From DUne F. Capozzi et al. (DUNE Collaboration), (2019),supplemental_material.pdf (aps.org)
    G0f=1.1663787e-11 ## MeV^-2 pg 313 griffiths
    vud=0.9738 ## also Grifiths 0.97270+-0.00014https://pdg.lbl.gov/2020/reviews/rpp2020-rev-ckm-matrix.pdf
    hbar=6.582e-22 ## MeV*s wiki 
...
max=make_cross_tgraph("max")
new=make_cross_tgraph("new")
min=make_cross_tgraph("min")
max.SetTitle("max")
new.SetTitle("2009 nuclear model")
min.SetTitle("min")
max.SetLineColor(ROOT.kRed)
min.SetLineColor(ROOT.kBlue)
new.SetLineColor(ROOT.kBlack)
mg.Add(max)
mg.Add(min)
mg.Add(new)
mg.Draw("AL")
leg = ROOT.TLegend(0.55, 0.35, 0.80, 0.50,""," NDC ")
leg.AddEntry(max, "GT strengths Max")
leg.AddEntry(new, "GT strengths (2009)")
leg.AddEntry(min, "GT strengths min")
leg.Draw()
